...
st.caption("Tamamlanan analizleri ve insan–fare ortolog yanıtlarını aynı yerde inceleyin.")
history_tab, species_tab = st.tabs(["Analiz geçmişi", "İnsan ↔ Fare"])
...
with species_tab:
    comparison = cross_species_comparison()
    if comparison.empty:
        st.info("Karşılaştırma için hem insan hem fare forward raporu gerekir. Her iki türde de en az bir simülasyon çalıştırın.")
    else:
        common = int((comparison["Karşılaştırma Durumu"] == "Her iki türde").sum())
        render_insight_box(
            f"İnsan ve fare raporları ortolog semboller üzerinden eşleştirildi; {common} kayıt her iki türde de bulundu. "
            "Hinterland skorları tür içi sıralamadır ve mutlak biyolojik etki olarak yorumlanmamalıdır."
        )
        metrics = st.columns(3)
        metrics[0].metric("Toplam eşleştirme", len(comparison))
        metrics[1].metric("Her iki türde", common)
        metrics[2].metric("Türe özgü", len(comparison) - common)
        render_dataframe(comparison, label="insan_fare_ortolog_karsilastirma", key="cross_species", row_limit=500)
